...
const pair<float, float> DAMP_RATE(0.91, 0.97); // velocity dampening (vel *= damp per update)
...
, _particleNoiseIndex(0) {
	...
	_particleQueue = dispatch_queue_create("relay.particles", DISPATCH_QUEUE_SERIAL);
}
